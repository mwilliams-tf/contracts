...
14. **UI Validation Recommendation**: If the feature includes UI components (check for files in `features/`, HTML templates, or routes in the plan):
    ...
    This feature includes UI components. Run `/speckit-validate` to verify that:
    - The implemented UI matches the quickstart.md documentation
    - Interactive elements work as documented
    - Labels, buttons, and flows align with documented expectations

    This step catches documentation drift and interaction model mismatches
